# core/utils/erp_detection_owlv2.py
import cv2
import torch
import numpy as np
...
from core.models.owlv2_client import OWLv2Engine
...
class ERPBoxDetector:
    ...
    Supports batch inference via OWLv2Engine.detect_batch for multiple tiles.
    """

    def __init__(
        self,
        det_engine: OWLv2Engine,
        cfg: PipelineConfig,
        prompt: List[str],
        ...
        """
        Args:
            owl_engine: Preconfigured OWLv2Engine.
            cfg:        PipelineConfig with mode, thresholds, face/nfov settings.
            prompt:     Text prompts for detection.
    ...
    def detect(self, frame_bgr: np.ndarray) -> List[Dict]:
        ...
        if mode == "full":
            ...
            raw_boxes, raw_scores, raw_phrases = self.det_engine.detect(
                pil,
                labels=self.prompt,
                threshold=self.cfg.owlv2_default_box_threshold,
            )
            ...
            boxes, scores, phrases = filter_face_detections(
                ...
                pil.height,
                max_face_frac=0.9,
                min_score=self.cfg.owlv2_default_box_threshold,
            )
        ...
        # ─── CUBEMAP MODE ──────────────────────────────────────────
            ...
            if len(images) > 1:
                ...
                rb, rs, rp = self.det_engine.detect(
                    images[0],
                    labels=self.prompt,
                    threshold=self.cfg.owlv2_default_box_threshold,
                )
                boxes_list, scores_list, phrases_list = [rb], [rs], [rp]
        ...
        # ─── NFOV MODE ─────────────────────────────────────────────
            ...
            if len(images) > 1:
                ...
                boxes_list, scores_list, phrases_list = self.det_engine.detect_batch(
                    images,
                    labels=self.prompt,
                    threshold=self.cfg.owlv2_nfov_box_threshold,
                )
            else:
                logger.debug("single NFoV detection")
                rb, rs, rp = self.det_engine.detect(
                    images[0],
                    labels=self.prompt,
                    threshold=self.cfg.owlv2_nfov_box_threshold,
...
class ERPFullPipeline:
    ...
    Uses OWLv2 for box detection (batched when multiple tiles),
    ...
    def process_frame(
        ...
        # 3) Run detection: batch if multiple tiles, else single with enhanced sensitivity
        # Use lower threshold for initial detection to catch more potential artifacts
        detection_threshold = max(0.01, self.cfg.owlv2_default_box_threshold * 0.5)
        logger.debug(f"Using enhanced detection threshold: {detection_threshold:.3f}")
        ...
        for (key, tile), raw_boxes, raw_scores, raw_phrases in zip(
            ...
            # 4a) filter out giant / low‐score with more lenient settings
            boxes, scores, phrases = filter_face_detections(
                raw_boxes,
                raw_scores,
                raw_phrases,
                tile.width,
                tile.height,
                max_face_frac=0.98,  # Allow even larger objects for better coverage
                min_score=self.cfg.owlv2_default_box_threshold * 0.5,  # More lenient score filtering
            )
            ...
            # 4b) optional SAM mask overlay (batch all boxes at once)
            ...
            if self.sam_client and len(boxes):
                ...
                for box in boxes:
                    ...
                    # First try with the configured threshold
                    ...
                    # If no masks found, try with an even lower threshold for better coverage
                    if not good_masks:
                        fallback_thresh = max(0.2, thresh * 0.5)  # Very lenient fallback
                        logger.debug(f"→ No masks ≥ {thresh:.3f}, trying fallback threshold {fallback_thresh:.3f}")
                        for mask, score in zip(mask_batch, mask_scores):
                    ...
                    if not good_masks:
                        logger.debug(f"→ no SAM masks ≥ {fallback_thresh:.3f} for box {coords}")
                    else:
                        logger.debug(f"→ merging {len(good_masks)} SAM masks for box {coords}")
                        ...
                        try:
                            ...
                            # Apply dilation to ensure complete coverage and no gaps
                            combined = ndimage.binary_dilation(combined, structure=kernel, iterations=2)
                            
                            # Apply erosion to smooth boundaries
        ...
        dets = filter_seam_artifacts(
            ...
            fw=W,
            fh=H,
            max_w_frac=seam_w_frac,
            max_h_frac=seam_h_frac,
        )
